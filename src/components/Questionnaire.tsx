...
export default function Questionnaire() {
  const [questions] = useState<QuestionQCM[]>(INITIAL_QUESTIONS);
      ...
      {isFinished ? (
        ...
        >
          ...
          <button
            ...
            onClick={handleReset}
            className="inline-flex items-center gap-2 px-6 py-2.5 rounded-lg bg-stone-900 text-white hover:bg-stone-800 text-sm font-medium transition-colors"
          >
              ...
              {currentQ.options.map((option, idx) => {
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-4 rounded-lg border text-sm md:text-base font-medium flex items-center justify-between transition-all ${optionStyle}`}
                  >
                    <span>{option}</span>
          ...
          <div id="questionnaire-footer-actions" className="flex items-center justify-end">
            {showResult && (
              <button
                ...
                onClick={handleNext}
                className="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-stone-900 text-white hover:bg-stone-800 text-sm font-medium transition-colors"
              >
