...
export default function ExercicesEcrits() {
  ...
  const handleDownload = () => {
    ...
    const currentScore = Object.values(evaluations).reduce((acc, curr) => acc + curr.score, 0);
    if (evaluatedCount > 0) {
  ...
  const totalEarnedScore = Object.values(evaluations).reduce((acc, curr) => acc + curr.score, 0);
                    ...
                    <div key={lineIdx} className="w-full flex items-center gap-2 group">
                      <div className="flex-1 relative flex items-center">
                        <input
                          ...
                          type="text"
                          value={lineText}
                          onChange={(e) => updateLine(item.id, lineIdx, e.target.value)}
